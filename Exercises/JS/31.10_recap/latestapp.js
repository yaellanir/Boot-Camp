...
const getAllCustomerCars = (customersArr, customerId) => {
  if (customerId) {
    const customer = customersArr.find((customer) => customer.id === customerId)
    console.log(customer);
    return customer.cars
  }
 const cars= customersArr.map((customer)=>{
    return customer.cars
  })
  return cars.flat()
};
console.log(getAllCustomerCars(ourCarMarket.customers ))
...
setNewCarToAgency(ourCarMarket.sellers[1],{
  brand : "Bmw",
  name: 'yael',
  year: 1984
});
...
const deleteCarFromAgency = (agencyObject, carNumber) => { 
  let itemToRemove =agencyObject.cars[carNumber]
  agencyObject.cars.splice(carNumber === itemToRemove)
  return itemToRemove;
};
...
const decOrIncCashOfAgency = (agencyObj, amount) => {
    const currentCash = agencyObj.cash += amount 
  return currentCash
};
decOrIncCashOfAgency(ourCarMarket.sellers[0], '2000')
